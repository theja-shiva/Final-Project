...
public class TrainerDao {
	...
	public int save(TrainerDetails t) {
		String sql = "insert into TR_details(tid,roomno,trName,platform,status) "
				+ "values(id_sq1.nextval,'"+ t.getRoomno()
				+ "','" + t.getTrName() + "','" + t.getPlatform() + "','" + t.getStatus() + "')";
		return template.update(sql);
	}
	
	public int update(TrainerDetails t) {
		 String sql="update TR_details set roomno='"+t.getRoomno()+"',trName='" + t.getTrName() + "',platform='" + t.getPlatform() +"',status='"+ t.getStatus() +"' where tid="+t.getTid()+"";
		return template.update(sql);
	 }
	...
	public TrainerDetails getRequestById(int tid) {
		...
		return template.queryForObject(sql, new Object[] {tid},new BeanPropertyRowMapper<TrainerDetails>(TrainerDetails.class));
	}
